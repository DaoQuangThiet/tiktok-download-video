...
const MemberScan = () => {
  ...
  useEffect(() => {
    ...
    //           setUrlState(true);
    //           setcurentUrl(url);
    if (localStorage.getItem('memberQty')) {
      const parseProfileQty = parseFloat(localStorage.getItem('memberQty'));
      currentQty.current = parseProfileQty;
      setMemberQty(currentQty.current);
    }
  ...
  const clickButtonGetProfile = async (paramsCurrent) => {
    ...
    if (localStorage.getItem('endCusorMembers')) {
      endCursorMembers = localStorage.getItem('endCusorMembers');
    }
    ...
    try {
      ...
      // toast.error('Không thể lấy số thành viên. Thử lại !', {
      //   position: 'top-right',
      //   autoClose: 5000,
      //   hideProgressBar: false,
      //   closeOnClick: true,
    ...
    while (hasNextPage && !stopCall.current) {
      ...
      try {
        ...
        if (responseData.errors) {
          localStorage.setItem('endCusorMembers', endCursorMembers);
          ...
          if (!responseData.data.node.new_members.page_info.has_next_page) {
            ...
            if (localStorage.getItem('endCusorMembers')) {
              localStorage.removeItem('endCusorMembers');
            }
            if (localStorage.getItem('memberQty')) {
              localStorage.removeItem('memberQty');
            }
            break;
          } else {
            endCursorMembers =
